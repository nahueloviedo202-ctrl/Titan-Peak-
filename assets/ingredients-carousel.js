...
/**
 * A multi-column carousel that slides a fixed number of cards at a time on
 * desktop (via a JS-driven translateX on the track) and becomes a native
 * touch-swipeable horizontal scroller on mobile (the track itself scrolls,
 * with CSS scroll-snap, like products-carousel). Visible card count is
 * derived from the rendered card width (set via CSS custom properties in the
 * section schema), so it stays correct at any breakpoint without hardcoded
 * widths in JS.
 *
 * @typedef {object} Refs
...
class IngredientsCarouselComponent extends Component {
   ...
   * (touch-swipeable, like products-carousel) instead of a JS-driven
   * translateX slider.
   */
  get #isMobile() {
