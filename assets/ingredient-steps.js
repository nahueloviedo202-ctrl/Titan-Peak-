...
/**
 * A multi-column "ingredient steps" carousel that pages a fixed number of
 * cards at a time on desktop (via a JS-driven translateX on the track) and
 * becomes a native touch-swipeable horizontal scroller on mobile (the track
 * itself scrolls, with CSS scroll-snap). Visible card count is derived from
 * the rendered card width (set via CSS custom properties in the section
 ...
 */
class IngredientStepsComponent extends Component {
...
if (!customElements.get('ingredient-steps-component')) {
  customElements.define('ingredient-steps-component', IngredientStepsComponent);
}
